...
public:
    ...
    void DrawInstanced(Shader &shader, unsigned int num_of_instances)
    {
        for(unsigned int i = 0; i < meshes.size(); i++)
            meshes[i].DrawInstanced(shader, num_of_instances);
    }

    void DrawPatched(Shader &shader)
    {
        for(unsigned int i = 0; i < meshes.size(); i++)
            meshes[i].DrawPatched(shader);
    }
