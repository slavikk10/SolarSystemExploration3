...
public:
    ...
    glm::vec3 Right;
    glm::vec3 WorldUp;
    glm::vec3 OrbitalCameraPosition;
    // euler Angles
    float Yaw;
    float Pitch;
    float Roll;
    ...
    {
        return glm::lookAt(glm::vec3(0.0f), static_cast<glm::vec3>(target - Position), static_cast<glm::vec3>(up));
    }
    ...
    {
        Zoom -= (float)yoffset * (Zoom / 20.0f);
        if (Zoom < 5.0f)
            Zoom = 5.0f;
        if (Zoom > 100000000000000.0f)
            Zoom = 100000000000000.0f;
    }
