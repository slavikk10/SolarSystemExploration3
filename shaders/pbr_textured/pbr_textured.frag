...
uniform bool skipRM;
//uniform bool containsNormalMap;
...
void main() {
    ...
    float roughness = 1.0 - max(texture(roughnessMap, TexCoords).r, 0.001);
    if (skipRM) {
        metallic = 0.0;
        roughness = 1.0;
    }
    ...
    FragColor = vec4(pow((ambient + Lo) / ((ambient + Lo) + vec3(1.0)), vec3(1.0/2.2)), texture(albedoMap, TexCoords).a);
}