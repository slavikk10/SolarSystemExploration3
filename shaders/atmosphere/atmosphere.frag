...
uniform float scatteringStrength = 400000.0;
...
void main() {
    ...
    float linearDepth = pow(2.0, texture(depthTex, TexCoords).r * 49.829) - 1.0;
    ...
    vec2 planetHitInfo = raySphere(planetWorldPos, planetRadius, vec3(0.0), viewDir);
    float dstToPlanet = planetHitInfo.x;

    float dstThroughAtmosphere;
    if (planetHitInfo.y > 0) {
